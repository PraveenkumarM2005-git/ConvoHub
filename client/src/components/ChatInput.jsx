...
const ChatInput = ({ selectedChat }) => {
  ...
  return (
    <div className="p-4 bg-dark-950/60 backdrop-blur-xl border-t border-dark-600 z-[60] relative">
      <form onSubmit={handleSend} className="flex items-end gap-2 relative">
        ...
        <button
          ...
          className="p-2.5 mb-0.5 rounded-lg text-dark-200 hover:text-white hover:bg-dark-800 transition-colors shrink-0 outline-none disabled:opacity-50"
          title="Attach File"
        >
          {isUploading ? (
        ...
        <button
            ...
            className={`p-2.5 mb-0.5 rounded-lg transition-colors shrink-0 outline-none ${showEmojiPicker ? 'text-purple-400 bg-dark-800' : 'text-dark-200 hover:text-white hover:bg-dark-800'}`}
            title="Emojis"
            disabled={isUploading}
        >
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="flex-1 bg-dark-950/40 backdrop-blur-md rounded-xl border border-white/5 focus-within:border-purple-500/30 focus-within:bg-dark-800/80 focus-within:shadow-[0_0_15px_rgba(139,92,246,0.1)] transition-all flex items-center mb-0.5 overflow-hidden">
            <textarea
                ...
                className="w-full bg-transparent border-none py-3 px-4 text-dark-50 placeholder-dark-400 focus:outline-none text-[15px] resize-none leading-relaxed"
                disabled={isUploading}
        ...
        <button
          ...
          className="btn-primary p-3.5 mb-0.5 rounded-xl shrink-0 outline-none flex items-center justify-center transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
        >
